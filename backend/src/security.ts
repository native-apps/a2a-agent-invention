 ...
 * Also converts relative URLs to absolute using WEBSITE_URL.
 */
let websiteUrlForLinks = "";

export function setWebsiteUrlForLinks(url: string | undefined) {
...
export function filterResponse(text: string): string {
  ...
  // WEBSITE_URL. When not set, links are left as-is.
  ...
  // The AI sometimes uses the agent endpoint subdomain for website page links.
  // This post-processing fixes those links without stripping the
  // agent endpoint info from the system prompt.
  //
  // Three patterns:
  //   1. a2a.yourdomain.com → domain          (public default)
  //   2. a2a.{anyDomain}    → {anyDomain}     (custom domain from WEBSITE_URL)
  filtered = filtered.replace(
