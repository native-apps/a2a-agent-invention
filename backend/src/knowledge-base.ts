...
export function setBusinessGoals(goalsJson?: string): void {
  ...
  try {
    ...
    businessGoalsBlock = [
      ...
      `They are ANSWER CONTEXT ONLY: use them to recognize intent, answer`,
      `"who can help with X" questions, and inform referrals when someone`,
      `explicitly asks. They are NOT a to-do list and NEVER a reason to act on`,
      `your own initiative.`,
      ``,
      `OUTREACH RULES:`,
      `- DO search neighbors + knock when a visitor's request is OUTSIDE your`,
      `  offering, or when they explicitly ask about neighbors/the network —`,
      `  fresh live answers beat memory. This is always correct behavior.`,
      `- Do NOT knock on your own initiative just to "advance" a goal —`,
      `  goal-driven outreach happens via the scheduled heartbeat.`,
      `If a goal names partners, codes, links, or terms, you may PRESENT them`,
      `in answers exactly as written. Never invent codes or terms.`,
      ``,
      `...lines`,
      `--- END BUSINESS GOALS ---`,
    ].join("\n");
...
const NEIGHBOR_TRIAGE_BLOCK = [
  ...
  `1. OURS FIRST — Does the visitor want a product or service that YOUR business`,
  `   actually OFFERS (your identity + description above)? If yes: answer as`,
  `   yourself. Knowing ABOUT a topic is not the same as OFFERING it — if the`,
  `   visitor wants someone to DO the thing (write it, design it, build it,`,
  `   manage it) and that service is not what your business sells, it is NOT`,
  `   yours — go to step 3. Never refer a visitor out for your own offering.`,
  `2. UNSURE — Could be either? Run neighbors_search (default scope) first —`,
  `   it is instant, free, and read-only. If an approved neighbor clearly`,
  `   offers it, refer; if the ask is really yours, answer yourself. Searching`,
  `   costs nothing and is never a mistake; guessing an answer for a service`,
  `   you don't sell IS.`,
  ...
  `   do — and knock them for fresh details when the visitor wants specifics.`,
  `   Nothing fits? Say so plainly.`,
  ``,
