...
export function setBusinessGoals(goalsJson?: string): void {
  ...
  try {
    ...
    businessGoalsBlock = [
      ...
      `your own initiative.`,
      ``,
      `HARD RULE — no proactive outreach: do NOT call neighbors_knock during`,
      `visitor conversations to "advance" a goal. Goal-driven outreach happens`,
      `EXCLUSIVELY via the scheduled heartbeat (the owner's automated`,
      `outreach). During a chat, knock ONLY when (a) the visitor's request is`,
      `outside your offering AND you need a partner to serve them, or (b) the`,
      `visitor explicitly asks you to contact a neighbor. If a goal names`,
      `partners, codes, links, or terms, you may PRESENT them in answers`,
      `exactly as written. Never invent codes or terms.`,
      ``,
      `...lines`,
      `--- END BUSINESS GOALS ---`,
    ].join("\n");
