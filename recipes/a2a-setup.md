...
3. **Connect NEAR Wallet panel (no terminal)** — a NEAR wallet is required for the registry (the onchain entry is owned by the wallet's account). The scoped-access-key path: ① Generate Neighbor Key (ed25519 keypair created in-app; Web Crypto, needs macOS 14+/Safari 17+ — the message suggests the CLI path otherwise); ② **Authorize in App** (v1.2.213) — opens the wallet's authorize page right in the app webview (MyNearWallet default — the legacy open protocol; the wallet redirects back to the agent worker's done-page at GET /neighbors/wallet-done, and if the app blocks in-app navigation the wizard says so and offers the fallback) or **Copy Wallet Link** to open in ANY browser (phone works) — approve “Add access key” keeping LIMITED; ③ Verify Connection (live RPC access_key_list — checks the key landed on their account, plus a welcome-back hint after returning from the wallet); ④ Register Onchain — the wizard signs the register/update transaction itself (auto-detects which via get_agent; register costs 0.01 Ⓝ refundable, update is free) and submits via FastNEAR. The key is stored in settings (neighborKeyPublic/neighborKeySecret) and is scoped to register/update/heartbeat + the website-list methods on the Neighbors registry only — it can never move funds. Same key later powers the worker heartbeat (future phase). Settings: neighborWalletUrl holds the chosen wallet preset's login URL. NOTE: this is NEAR's native wallet login (requestSignIn URL standard) — NOT walletconnect.com (unrelated). **Meteor REMOVED from presets (2026-08-27):** Meteor's web wallet dropped the legacy /login protocol (it redirects to their create-wallet funnel; their new sign-in schema only fires via their widget postMessage flow — verified against their production bundle). In-app Meteor auth is impossible on macOS anyway (Tauri = WKWebView, no extensions). Meteor path forward: hosted connect page (Wallet Selector + AddKey) — see HANDOFF-IN-APP-WEBVIEW-TO-MB-CODER.md. GOTCHAS (caught live 2026-08-25): (a) the wallet must be signed in as the SAME account as the wizard's nearAccountId — approving while logged in as a different account adds the key THERE (it landed on neighborly.testnet instead of anakimota.testnet); (b) legacy MyNearWallet can grant FULL ACCESS despite the scoped link (its authorize screen is user-toggleable) — the wizard's Verify/Sign steps now check the onchain permission type and refuse over-permissioned keys with removal instructions; (c) approve keeping the LIMITED access option only.
...
- **Tags** — the user's own curated lists; a tag IS a list (filter the registry by #saas → the "SaaS" list). Up to 8 on the registry entry; unlimited locally.
- **Registry truth for support**: `status: 0` = active, `1` = paused (set via `heartbeat(Some(status))`). Reads are FREE public RPC (`https://test.rpc.fastnear.com`, contract `neighborly.testnet`) — but queries MUST pass `finality: "optimistic"` or they return empty/error. View methods: `get_agent(account)`, `get_agents(from_index, limit)`, `get_stats()`. Storage stake: 0.01 Ⓝ (refunded on unregister); updates are free and signed by the scoped neighbor key.
