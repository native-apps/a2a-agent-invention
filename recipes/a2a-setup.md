...
Registry: the NEAR Neighbors smart contract — **MAINNET since 2026-08: `nearneighbors.near`** (testnet `neighborly.testnet` is legacy/manual-tests only). Protocol over platform: free public reads, signer-scoped writes, storage-stake deposit (0.01 Ⓝ, refunded on unregister), zero admin powers. Contract source: near-contract/ in the repo (public, verifiable).
...
1. **Copy Registration Command** — generates the exact verified near-cli command from the profile fields (register call, 0.01 Ⓝ deposit, sign-as their NEAR account). Prereqs shown on-slide: a NEAR **mainnet** account in any wallet (MyNearWallet default; sunsets Oct 2026). When **nearAccountId** is already set, an additional **Copy Update Command** button appears (v1.2.168+) — the free, no-deposit `update` variant for already-registered accounts (running `register` again fails "already registered — use update()").
...
3. **Connect NEAR Wallet panel (no terminal)** — a NEAR wallet is required for the registry (the onchain entry is owned by the wallet's account). The scoped-access-key path: ① Generate Neighbor Key (ed25519 keypair created in-app; Web Crypto, needs macOS 14+/Safari 17+ — the message suggests the CLI path otherwise); ② **Authorize in App** (v1.2.213/214) — opens the wallet's authorize page from the app itself: child-window first (wizard stays visible), falling back to navigating the app webview, with the worker's done-page (GET /neighbors/wallet-done) offering a sanitized “Return to Mother Brain” button + 6s auto-return. Verified live in-app 2026-08-27: full-webview navigation works (child window blocked in current app builds), and MyNearWallet completed an authorization end-to-end. First MNW visit needs “Import Existing Account” (seed phrase); the pending authorize request can be dropped by MNW's login gate — if no authorize screen appears after import, click Authorize in App again (signed in now → authorize appears immediately). Fallback: **Copy Wallet Link** to open in ANY browser — approve “Add access key” keeping LIMITED; ③ Verify Connection (live RPC access_key_list — checks the key landed on their account; refuses FULL-ACCESS keys and wrong-contract keys, plus a welcome-back hint after returning from the wallet); ④ Register Onchain — the wizard signs the register/update transaction itself (auto-detects which via get_agent; register costs 0.01 Ⓝ refundable, update is free) and submits via FastNEAR. The key is stored in settings (neighborKeyPublic/neighborKeySecret) and is scoped to the Neighbors registry contract only — it can never move funds. Same key later powers the worker heartbeat (future phase). Settings: neighborWalletUrl holds the chosen wallet preset's login URL. NOTE: this is NEAR's native wallet login (requestSignIn URL standard) — NOT walletconnect.com (unrelated). **Meteor REMOVED from presets (2026-08-27):** Meteor's web wallet dropped the legacy /login protocol — /login now redirects to their create-wallet funnel and NO authorize screen ever renders (verified against their production bundle; their new sign-in schema only fires via their widget postMessage flow, never a URL visit). In-app Meteor auth is impossible on macOS anyway (Tauri = WKWebView, no extensions). A stale saved Meteor URL is auto-refused by the wizard (v1.2.214 guard). Meteor path forward: hosted connect page (Wallet Selector + AddKey) — see HANDOFF-IN-APP-WEBVIEW-TO-MB-CODER.md. GOTCHAS: (a) the wallet must be signed in as the SAME account as the wizard's nearAccountId — approving while logged in as a different account adds the key THERE (caught live 2026-08-25: landed on neighborly.testnet instead of anakimota.testnet); (b) MyNearWallet's authorize screen permission toggle is loose — it can grant FULL ACCESS despite the scoped link, and observed live 2026-08-27 it can also drop the method list (key added receiver-scoped with EMPTY method_names = any method on the registry contract; acceptable — cannot move funds or touch other contracts — but re-approve keeping LIMITED + methods when possible). Verify/Sign refuse over-permissioned (FullAccess / wrong-contract) keys with removal instructions; (c) approve keeping the LIMITED access option only.
...
3. **NEAR account set** — valid mainnet format (`name.near`, subaccounts, or a 64-hex implicit account).
...
### Neighbors CRM (the business side — crm/NeighborsView.tsx)
The Neighbors tab (CRM) is where the owner runs the business workflow after registration. The node's AI Assistant should guide users through these:

- **Goals** — enabled goals feed the Heartbeat outreach engine: the backend cron fires every 30 min (`*/30 * * * *` in backend/wrangler.toml), gated by the owner's schedule (`HEARTBEAT_SCHEDULE_JSON`: interval (1-24h) | daily+time | weekly+day, timezone-aware). Per run: one knock, deterministic goal rotation, targets from the owner's curated list only (no Knick pool yet). Knock text is composed from the goal (title + body brief) — no LLM call on the sender side.
- **Deals** — the business's own partnership offers (status: draft → …). Summarized into the SOP-generation prompt.
- **SOPs** — playbooks the agent follows in agent-to-agent conversations. **AI Generate** drafts 2-4 SOPs from enabled Goals + Deals (never invents coupon codes, prices, or terms; bodies are numbered steps under ~120 words). FIXED 2026-08-27: generation budget raised 1200 → 3000 tokens and `parseLlmJson` now closes truncated JSON (string/bracket-aware `closeTruncatedJson` candidates) — replies cut mid-array by `finish_reason: "length"` now parse, and the title/body filter drops any dangling partial draft. The old "AI returned unparseable output — try again" error from truncation is gone. **Improve** (per-SOP) asks for a `TITLE:`/`BODY:` plain-text reply with a JSON-object fallback.
- **Tags** — the user's own curated lists; a tag IS a list (filter the registry by #saas → the "SaaS" list). Up to 8 on the registry entry; unlimited locally. **Approved-only mentions (v1.2.211):** the agent may ONLY mention/recommend neighbors on PUBLISHED tag-lists (the onchain named lists shown on the website) — `neighbors_search` defaults to scope "approved" (whole-network scope "all" fires only on an explicit user request, presented as directory info); an INBOUND TRIAGE prompt block runs in every conversation (ours-first → one clarifying question when unsure → refer approved only → never recommend an offering that overlaps ours; no approved neighbors = mention none). The worker knows its curator via the `NEIGHBORS_CURATOR` var (deployed from nearAccountId). Fail-closed: chain unreadable/no lists → recommend nobody, never widen to the raw registry. If a list read fails partially it stays ok (every entry is individually approved).
- **Registry truth for support**: `status: 0` = active, `1` = paused (set via `heartbeat(Some(status))`). Reads are FREE public RPC (`https://rpc.fastnear.com`, contract `nearneighbors.near` — MAINNET since the 2026-08 swap; testnet `neighborly.testnet` is legacy); `finality: "final"` works (verified repeatedly 2026-08-27) and `"optimistic"` also works. View methods: `get_agent(account)`, `get_agents(from_index, limit)`, `get_stats()`, plus the named-list views `get_named_lists(curator)` / `get_named_list(curator, slug)`. Storage stake: 0.01 Ⓝ (refunded on unregister); updates are free and signed by the scoped neighbor key.
- Verified live 2026-08-27 (testnet era): `anakimota.testnet` registered AND active, full profile set. Superseded by the 2026-08 mainnet swap — first mainnet registrations: `anakimota.near` + `motherbrain.near`.
