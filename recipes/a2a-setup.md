...
1. **Copy Registration Command** — generates the exact verified near-cli command from the profile fields (register call, 0.01 Ⓝ deposit, sign-as their NEAR account). Prereqs shown on-slide: a NEAR testnet account in any wallet (Meteor recommended; MyNearWallet sunsets Oct 2026). When **nearAccountId** is already set, an additional **Copy Update Command** button appears (v1.2.168+) — the free, no-deposit `update` variant for already-registered accounts (running `register` again fails "already registered — use update()").
