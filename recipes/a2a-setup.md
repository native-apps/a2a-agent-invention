...
1. **Copy Registration Command** — generates the exact verified near-cli command from the profile fields (register call, 0.01 Ⓝ deposit, sign-as their NEAR account). Prereqs shown on-slide: a NEAR testnet account in any wallet (MyNearWallet default; sunsets Oct 2026). When **nearAccountId** is already set, an additional **Copy Update Command** button appears (v1.2.168+) — the free, no-deposit `update` variant for already-registered accounts (running `register` again fails "already registered — use update()").
...
- **Registry truth for support**: `status: 0` = active, `1` = paused (set via `heartbeat(Some(status))`). Reads are FREE public RPC (`https://test.rpc.fastnear.com`, contract `neighborly.testnet`); `finality: "final"` works (verified repeatedly 2026-08-27) and `"optimistic"` also works. View methods: `get_agent(account)`, `get_agents(from_index, limit)`, `get_stats()`, plus the named-list views `get_named_lists(curator)` / `get_named_list(curator, slug)`. Storage stake: 0.01 Ⓝ (refunded on unregister); updates are free and signed by the scoped neighbor key.
- Verified live 2026-08-27: `anakimota.testnet` is registered AND active — name, domain (agentext.pro), agent_url (https://a2a.agentext.pro), website_url, description, 6 tags, 8 capabilities, partner_note all set; `updated_at` > `registered_at` (entry has been maintained).
