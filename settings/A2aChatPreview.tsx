...
const A2aChatPreview: React.FC<A2aChatPreviewProps> = ({ invention }) => {
  ...
  // ── HERO MODE (Hero Search — initial collapsed state) ──
  if (mode === "hero") {
    const handleHeroKeyDown = (e: React.KeyboardEvent) => {
      if (e.key === "Enter") {
        e.preventDefault();
        handleHeroSubmit(heroInput);
      }
    };
    ...
    return (
      ...
      >
        ...
        <div style={{ marginBottom: 24 }}>
          <BrainLogo size={48} color={T.neonGreen} logoUrl={cfg.logoUrl} />
        </div>
        ...
        >
          {cfg.agentDescription}
        </div>

        {/* Search Input — Octagonal Hero Search */}
          ...
          >
            ...
            <div
              style={{
                position: "absolute",
                inset: -2,
                background: `linear-gradient(135deg, ${T.neonGreen}40, ${T.hotPink}30, ${T.electricCyan}40)`,
                padding: 2,
                clipPath:
                  "polygon(8% 0%, 92% 0%, 100% 30%, 100% 70%, 92% 100%, 8% 100%, 0% 70%, 0% 30%)",
              }}
            >
              <div
                style={{
            ...
            >
              ...
              <svg
                ...
                style={{ marginRight: 12, flexShrink: 0, opacity: 0.8 }}
              >
              ...
              <input
                ...
                placeholder={heroInput ? "" : heroDisplayed}
                style={{
                  flex: 1,
                  ...
                  fontFamily: T.font,
                  color: T.text,
                  caretColor: T.neonGreen,
                }}
              />
              {/* Enter indicator */}
              {heroInput.trim() && (
                <button
                  onClick={() => handleHeroSubmit(heroInput)}
                  style={{
                    background: "transparent",
                    border: "none",
                    color: T.neonGreen,
        ...
        <button
          ...
          style={{
            ...
            fontFamily: T.font,
            fontWeight: "bold",
            color: heroInput.trim() ? T.deepVoid : T.textMuted,
            background: heroInput.trim() ? T.neonGreen : T.neuralNode,
            border: "none",
            cursor: heroInput.trim() ? "pointer" : "default",
            clipPath:
              "polygon(6% 0%, 94% 0%, 100% 50%, 94% 100%, 6% 100%, 0% 50%)",
            transition: "background 0.2s, color 0.2s",
            opacity: heroInput.trim() ? 1 : 0.5,
          }}
        >
          Ask {cfg.agentName}
        </button>

        {/* If chat history exists, show a link to open it */}
        {messages.length > 0 && (
          <button
