...
// absolutizeUrls is created dynamically inside the component using settings
...
function getSettings(settings: Record<string, unknown>) {
  return {
    ...
    agentDescription: (settings.agentDescription as string) || "AI assistant",
    agentUrl: (settings.agentUrl as string) || "",
    widgetColor: (settings.widgetColor as string) || "#39ff14",
    widgetBranding: (settings.widgetBranding as string) || "",
...
async function fetchHistoryFromSupabase(
  ...
  try {
    const endpointUrl = agentUrl || "";
    if (!endpointUrl) return { messages: [], hasMore: false };
...
const A2aChatPreview: React.FC<A2aChatPreviewProps> = ({ invention }) => {
  ...
  const endpointUrl = cfg.agentUrl || "";

  // Dynamic base URL for link absolutization (from settings — not hardcoded)
  const baseUrl =
    ((invention.settings as Record<string, unknown>).websiteUrl as string) ||
    endpointUrl.replace(/^(https?:\/\/[^/]+).*/, "$1") ||
    "";
                    ...
                    (msg.role === "user" ? (
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html:
                            renderMarkdown(absolutizeUrls(msg.text)) +
