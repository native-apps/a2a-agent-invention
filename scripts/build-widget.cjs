...
// ── Paths ────────────────────────────────────────────────────────────────
const TEMPLATE_PATH = path.resolve(__dirname, "..", "frontend", "bundle", "motherbrain-chat.js");
...
function applyConfig(template, config, logoDataUrl) {
  ...
  // Default endpoint
  const endpoint = config.endpoint || config.agentUrl || "https://a2a.motherbrain.app";
  ...
  const branding = config.branding || config.widgetBranding || "Powered by Mother Brain";
  result = result.replace(
...
<body>
  <h1>${agentName}</h1>
  <p>
    This is a preview of the <strong>${agentName}</strong> chat widget.
    Click the button below or the bottom bar to open the chat.
  </p>
...
function main() {
  ...
  console.log(`  Endpoint: ${config.endpoint || config.agentUrl || "https://a2a.motherbrain.app"}`);
  console.log(`  Theme:    ${config.theme || "dark"}`);
  console.log(`  Color:    ${config.widgetColor || config.primaryColor || "#39ff14"}`);
