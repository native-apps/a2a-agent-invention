...
class NeHeroSearch extends HTMLElement {
  ...
  constructor() {
    ...
    */
    ...
    const strokeGradWrap = document.createElementNS(ns, "svg");
    ...
    Array.from(strokeGradWrap.children).forEach((n) =>
      this.defs.appendChild(n),
    );

    // Brain gradient (green → purple)
    ...
    Array.from(brainGradWrap.children).forEach((n) => this.defs.appendChild(n));
    ...
    this.bg.setAttribute("stroke", "url(#amberGlow)");
    this.bg.setAttribute("stroke-width", "4");
    this.bg.setAttribute("stroke-linejoin", "round");
    this.bg.setAttribute("filter", "url(#amberGlowFilter)");
  ...
  connectedCallback() {
    ...
    this.brain.addEventListener("click", (e) => {
      e.stopPropagation();
      // If autoTyping: use completedQuery (last fully-typed suggestion)
      // If user typed their own: use editor.value
      const query = this.autoTyping
        ? (this.completedQuery || "").trim()
    ...
    this.addEventListener("mouseenter", () => {
      if (this.autoTyping && this.enhanced && !this.enhanced.isPaused()) {
        this.enhanced.pause();
      }
    });
  ...
  private startSuggestions() {
    ...
    this.enhanced = new EnhancedTyped(this.editor, {
      strings: suggestions,
      loop: true,
      nextStringDelay: 3500,
      onCharTyped: (v, isComplete) => {
        this.editor.value = v;
        this.lastQuery = v;
        // SHARP: update completedQuery the exact moment the last char is typed
        if (isComplete) this.completedQuery = v;
      },
    });
  }
