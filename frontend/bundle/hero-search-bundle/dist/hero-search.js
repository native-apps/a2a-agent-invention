...
(() => {
  ...
  class EnhancedTyped {
    ...
    simulateUserInput(newValue) {
      const input = this.el;
      try { input.dataset.enhancedFocus = "1"; input.focus(); } catch {}
      try {
      ...
      try { queueMicrotask(() => { try { delete input.dataset.enhancedFocus; } catch {} }); } catch {}
    }
  ...
  class NeHeroSearch extends HTMLElement {
    constructor() {
      ...
      const applyTheme = () => {
        ...
        const op1 = styles.getPropertyValue("--hero-search-stop1-opacity").trim() || "0.2";
        const op2 = styles.getPropertyValue("--hero-search-stop2-opacity").trim() || "0.4";
    ...
    connectedCallback() {
      ...
      editor.addEventListener("keydown", (e) => {
        if (e.key === "Escape") {
          e.preventDefault();
          e.stopPropagation();
          this._brain.dispatchEvent(new PointerEvent("click"));
        }
      });
      ...
      editor.addEventListener("touchstart", onEditorTap, { capture: true, passive: true });
      ...
      // Brain icon click resets suggestions
      ...
      // ResizeObserver
      this._resizeObserver = new ResizeObserver(([{ contentRect: { width } }]) => {
      ...
      this._foreignObj.addEventListener("pointerdown", this._stopSuggestions.bind(this));
    }
    ...
    _updateGeometry(width) {
      ...
      this._svg.querySelectorAll("path").forEach((p) => {
        ...
        try { if (p.getBBox().width < 100) return; } catch { return; }
    ...
    _applyBorderStyle(styleId) {
      ...
      if (snippet) {
        ...
        const map = {
          AMBER_BORDER: { stroke: "url(#hs-amberGlow)", filter: "url(#hs-amberGlowFilter)" },
